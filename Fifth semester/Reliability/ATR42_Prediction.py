...
df = pd.read_csv('dataBaseColombianAviationNavy_C208B-FINAL.csv')
...
df.index.freq='MS'
'''
figATR = px.line(df, x=df.index, y='Number_of_incidents', template="plotly_dark", title="C-208B Grand Caravan Frequency of Accidents by Month of the NAVY")
figATR.show()
'''
Prueba_Dickey_Fuller(df['Number_of_incidents'],'Number_of_incidents') #the data are stationary


# Eliminar la primera fila (que contendrá un valor NaN debido a la diferenciación)
train_dataATR42=df[:len(df)-5]
test_data=df[len(df)-5:]
test=test_data.copy()
#Prueba_Dickey_Fuller(df['Number_of_incidents_diff'],'Number_of_incidents')
...
'''
arima_model = SARIMAX(train_dataATR42["Number_of_incidents"], order = (3,1,1), seasonal_order = (2,1,1,12)) 
...
plt.title("C208B NAVY", fontsize=30)
plt.xlabel('Meses')
plt.ylabel('')
